...
func MustLoad() *Config {
	once.Do(func() {
		...
		// 1. Сначала пытаемся загрузить базовые настройки из .env, если он есть
		...
		// 2. ВСЕГДА читаем переменные окружения поверх!
		// Если мы в Docker, значения из docker-compose.yml гарантированно перекроют файл .env
